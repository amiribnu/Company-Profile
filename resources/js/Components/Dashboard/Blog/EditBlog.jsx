...
export default function EditBlog() {
    const { posts, categories, tags } = usePage().props;
    const [imagePreview, setImagePreview] = useState(null);
    const [isQuillContentEmpty, setIsQuillContentEmpty] = useState(true);
    const imageRef = React.useRef(null);
    const [title, setTitle] = useState(posts.title);
    const [short_desc, setShort_desc] = useState(posts.short_desc);
    const [category_id, setCategory_id] = useState(posts.category_id);
    const { data, setData, errors } = useForm({
        body: posts.body,
    });
    const [selectedTags, setSelectedTags] = React.useState(
        posts.tags.map((tag) => tag.name)
    );
    ...
    React.useEffect(() => {
        if (quillRef.current) {
            ...
            quill.root.innerHTML = posts.body;
    ...
    function handleSubmit(e) {
        ...
        router.post(
            route("dashboard.blog.update", { blog: posts.id }),
            formData,
            {
                // notif
                onSuccess: () => {
                            ...
                            <div className="sm:col-span-10">
                                ...
                                {posts.image ? (
                                    <img
                                        ...
                                        alt="Current Image"
                                        className={`object-cover h-48 ml-4 ${
                                            imagePreview && "hidden"
                                        }`}
                                        loading="lazy"
                                    />
